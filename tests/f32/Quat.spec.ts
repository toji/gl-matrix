...
describe('Quat', () => {
  describe('constructor', () => {
    it('should return Quat(0, 0, 0, 1) if called with no arguments', () => expect(new Quat()).toBeVec(0, 0, 0, 1));
  ...
  describe('static', () => {
    ...
    describe('slerp', () => {
      describe('the normal case', () => {
        beforeEach(() => { result = Quat.slerp(out, [0, 0, 0, 1], [0, 1, 0, 0], 0.5); });

        it('should return out', () => expect(result).toBe(out));
        it('should calculate proper quat', () => expect(result).toBeVec(0, 0.707106, 0, 0.707106));
      });

      describe('where a == b', () => {
        beforeEach(() => { result = Quat.slerp(out, [0, 0, 0, 1], [0, 0, 0, 1], 0.5); });
      ...
      describe('where a == -b', () => {
        beforeEach(() => { result = Quat.slerp(out, [1, 0, 0, 0], [-1, 0, 0, 0], 0.5); });

        it('should return out', () => expect(result).toBe(out));
        it('should calculate proper quat', () => expect(result).toBeVec(1, 0, 0, 0));
      });
    });

    describe('pow', () => {
      describe('identity quat', () => {
        beforeEach(() => { result = Quat.pow(out, id, 2.1 /* random number */); });

        it('should return out', () => expect(result).toBe(out));
        it('should be the identity', () => expect(result).toBeVec(id));
      });
    ...
    describe('rotateX', () => {
      beforeEach(() => { result = Quat.rotateX(out, id, deg90); });
    ...
    describe('rotateY', () => {
      beforeEach(() => { result = Quat.rotateY(out, id, deg90); });
    ...
    describe('rotateZ', () => {
      beforeEach(() => { result = Quat.rotateZ(out, id, deg90); });
    ...
    /*
      ...
      describe('legacy', () => {
        ...
        it('should set dest to the correct value', () => expect(result).toBeVec(-0.707106, 0, 0, 0.707106));
      });
      ...
      describe('from a normal matrix looking `backward`', () => {
        beforeEach(() => {
          matr = mat3.create();
          mat3.transpose(matr, mat3.invert(matr, mat3.fromMat4(matr, mat4.lookAt(
           mat4.create(), [0, 0, 0], [0, 0, 1], [0, 1, 0]))));
          result = Quat.fromMat3(out, matr);
        });

        it('should return out', () => expect(result).toBe(out));

        it('should produce the same transformation as the given matrix', () => expect(Vec3.transformQuat([], [3,2,-1],
         Quat.normalize(out, out))).toBeEqualish(Vec3.transformMat3([], [3,2,-1], matr)));
      });
    ...
    describe('fromEuler', () => {
      describe('legacy', () => {
        beforeEach(() => { result = Quat.fromEuler(out, -90, 0, 0); });
    ...
    /*
      ...
      describe('given opengl defaults', () => {
        ...
        it('should return out', () => expect(result).toBe(out));
        it('should produce identity', () => expect(out).toBeVec(0, 0, 0, 1));
      });
      ...
      describe('legacy example', () => {
        ...
        it('should set correct quat4 values', () => expect(result).toBeVec(0.707106, 0, 0, 0.707106));
      });
    });*/
    ...
    describe('rotationTo', () => {
      ...
      describe('at right angle', () => {
        beforeEach(() => { result = Quat.rotationTo(out, [0, 1, 0], [1, 0, 0]); });

        it('should return out', () => expect(result).toBe(out));
        it('should calculate proper quaternion', () => expect(out).toBeVec(0, 0, -0.707106, 0.707106));
      });

      describe('when vectors are parallel', () => {
        beforeEach(() => { result = Quat.rotationTo(out, [0, 1, 0], [0, 1, 0]); });

        it('should return out', () => expect(result).toBe(out));
        it('multiplying A should produce B', () => expect(Vec3.transformQuat(r, [0, 1, 0], out)).toBeVec(0, 1, 0));
      });

      describe('when vectors are opposed X', () => {
        beforeEach(() => { result = Quat.rotationTo(out, [1, 0, 0], [-1, 0, 0]); });

        it('should return out', () => expect(result).toBe(out));
        it('multiplying A should produce B', () => expect(Vec3.transformQuat(r, [1, 0, 0], out)).toBeVec(-1, 0, 0));
      });
      ...
      describe('when vectors are opposed Z', () => {
        beforeEach(() => { result = Quat.rotationTo(out, [0, 0, 1], [0, 0, -1]); });

        it('should return out', () => expect(result).toBe(out));
        it('multiplying A should produce B', () => expect(Vec3.transformQuat(r, [0, 0, 1], out)).toBeVec(0, 0, -1));
      });
    });
    ...
    describe('getAxisAngle', () => {
      ...
      describe('for a simple rotation about Y axis', () => {
        beforeEach(() => {
          ...
          angle = Quat.getAxisAngle(vec, out);
        });
      ...
      describe('for a very irregular axis and negative input angle', () => {
        beforeEach(() => {
          quatA = Quat.setAxisAngle(quatA, [0.65538555, 0.49153915, 0.57346237], 8.8888) as Quat;
          angle = Quat.getAxisAngle(vec, quatA);
          quatB = Quat.setAxisAngle(quatB, vec, angle) as Quat;
        });

        it('should return an angle between 0 and 2*PI', () => {
          expect(angle).toBeGreaterThan(0.0);
          expect(angle).toBeLessThan(Math.PI * 2.0);
        });
    ...
    describe('getAngle', () => {
      describe('from itself', () => {
        beforeEach(() => { Quat.normalize(quatA, quatA); });

        it('should be zero', () => expect(Quat.getAngle(quatA, quatA)).toBeCloseTo(0));
      });
      ...
      describe('from rotated', () => {
        ...
        it('should be 45 degrees', () => expect(Quat.getAngle(quatA, quatB)).toBeCloseTo(Math.PI / 4));
      });
